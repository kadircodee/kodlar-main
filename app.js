...
mongoose.connection.on("error", () => {
	console.error("Database'e Bağlanılamadı!");
});
// </> DB Connection </>
...
app.get("/profile/:userID", async (req, res) => {
  ...
  let auth;
  if (member.roles.cache.has(conf.ownerRole)) auth = "Sunucu Sahibi";
  else if (member.roles.cache.has(conf.adminRole)) auth = "Yönetici";
  else if (member.roles.cache.has(conf.codeSharer)) auth = "Kod Paylaşım Ekibi";
  else if (member.roles.cache.has(conf.booster)) auth = "Takviyeci";
  else auth = "Üye";
  res.render("profile", {
    user: req.user,
    member,
...
app.get("/normal/:codeID", async (req, res) => {
  ...
  const guild = client.guilds.cache.get(conf.guildID);
  const member = req.user ? guild.members.cache.get(req.user.id) : null;
  if (member && !member.roles.cache.has(conf.normalRole) && !member.roles.cache.has(conf.booster) && !member.roles.cache.has(conf.ownerRole) && !member.roles.cache.has(conf.adminRole)) return error(res, 501, "Bu kodu görebilmek için gerekli rolleriniz bulunmamaktadır! Lütfen bilgilendirme sayfasını okuyunuz!");
...
app.get("/edit/:rank/:id", async (req, res) => {
  ...
  res.render("editCode", {
    user: req.user,
    icon: client.guilds.cache.get(conf.guildID).iconURL({ dynamic: true }),
    isStaff: client.guilds.cache.get(conf.guildID).members.cache.get(req.user.id).roles.cache.has("962512088775946357"),
    reqMember: req.user ? client.guilds.cache.get(conf.guildID).members.cache.get(req.user.id) : null,
